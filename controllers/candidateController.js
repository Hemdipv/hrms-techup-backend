...
const addCandidate = async (req, res, next) => {
  ...
  try {
    ...
    console.log("response", info, req.files);
    ...
    if (!name || !mobile || !resume || !createdBy || !dateOfApplication) {
      ...
      if (aadhaar) {
        fs.unlink(aadhaar[0]?.path, function (err) {
          if (err) return console.log(err);
      ...
      if (experienceLetter) {
        fs.unlink(experienceLetter[0]?.path, function (err) {
          if (err) return console.log(err);
          console.log("experienceLetter file deleted successfully");
        });
      }
      if (marksheet) {
      ...
      if (companyofferletter) {
        fs.unlink(companyofferletter[0]?.path, function (err) {
          if (err) return console.log(err);
          console.log("companyofferletter file deleted successfully");
        });
      }
